...
{
	std::tuple<float, float> texels = _getTexelIndexes(polygon, w0, w1, w2, texture);

	int pixel_index = (int)std::get<1>(texels) * texture.textureWidth * texture.textureChannels + (int)std::get<0>(texels) * texture.textureChannels;
...
std::tuple<float, float, float> Rasterizator::_getMRAO(const Triangle& polygon, float w0, float w1, float w2,
	TextureStruct& texture)
{
	std::tuple<float, float> texels = _getTexelIndexes(polygon, w0, w1, w2, texture);

	int texelX = (int)std::get<0>(texels);
	int texelY = (int)std::get<1>(texels);

	int MapPixelIndex = (int)texelY * texture.textureWidth * texture.textureChannels + (int)texelX * texture.textureChannels;
	...
	return std::make_tuple(metalness, roughness, ao);
}

RGBQUAD Rasterizator::_calculateBillinearColor(const Triangle& polygon, float w0, float w1, float w2,
	TextureStruct& texture)
{
	std::tuple<float, float> texels = _getTexelIndexes(polygon, w0, w1, w2, texture);

	//  Пиксельные координаты текстуры (texX, texY) для четырех ближайших текселей вокруг (u, v)
	/*int x1 = (int)floor(std::get<0>(texels));
	int y1 = (int)floor(std::get<1>(texels));
	int x2 = (int)ceil(std::get<0>(texels));
	int y2 = (int)ceil(std::get<1>(texels));*/

	int x1 = (int)std::get<0>(texels);
	int y1 = (int)std::get<1>(texels);
	int x2 = x1 + 1;
	int y2 = y1 + 1;
	...
	float int_part;
	float s = std::modf(std::get<0>(texels), &int_part);
	float t = std::modf(std::get<1>(texels), &int_part);
...
CoordinateStruct Rasterizator::_getNormalMap(float texX, float texY, TextureStruct& texture)
{
...
std::tuple<float,float> Rasterizator::_getTexelIndexes(const Triangle& polygon, float w0, float w1, float w2, TextureStruct& texture)
{
	...
	return std::make_tuple(texelX, texelY);
}
...
void Rasterizator::DrawPolygonBase(const Triangle& polygon, std::vector<PointLightStruct> lightnings,
	...
	void* depthBuffer, std::function<RGBQUAD(float, float, float, float, float, const Triangle&,
		...
		for (int x = rect.left; x <= rect.right; x++)
		{
			...
			if ((w0 >= 0 && w1 >= 0 && w2 >= 0) || (w0 < 0 && w1 < 0 && w2 < 0))
			{
				...
				if (z < reinterpret_cast<float*>(depthBuffer)[y * FrameWidth + x])
				{
					...
					RGBQUAD lightedColor = calculateColor(curXInGlobal, curYInGlobal, w0, w1, w2, polygon, curPointInGlobal, hitNormal, lightnings);
...
{
	auto calculateColor = [&](float x, float y, float, float, float, const Triangle& polygon, CoordinateStruct& curPointInGlobal, CoordinateStruct& hitNormal, std::vector<PointLightStruct>& lightnings)
		{
...
{
	auto calculateColor = [&](float x, float y, float w0, float w1, float w2, const Triangle& polygon, CoordinateStruct& curPointInGlobal, CoordinateStruct& hitNormal, std::vector<PointLightStruct>& lightnings)
		{
			RGBQUAD finalColor = _calculateTextureColor(polygon, w0, w1, w2, texture, x, y);
			return finalColor;
		};
...
void Rasterizator::DrawPolygonBarycentricTextureWithLight(const Triangle& polygon, std::vector<PointLightStruct> lightnings, CoordinateStruct& CameraGlobalCoordinates,
	void* frameBuffer, void* depthBuffer, RGBQUAD color, TextureStruct& texture)
	...
	auto calculateColor = [&](float x, float y, float w0, float w1, float w2, const Triangle& polygon, CoordinateStruct& curPointInGlobal, CoordinateStruct& hitNormal, std::vector<PointLightStruct>& lightnings)
		{
			...
			RGBQUAD texelColor = _calculateTextureColor(polygon, w0, w1, w2, texture, x, y);
			CoordinateStruct texelStruct = { texelColor.rgbBlue,texelColor.rgbGreen, texelColor.rgbRed };
			return _getLightedColor(texelStruct, hitColor);
		};

	DrawPolygonBase(polygon, lightnings, CameraGlobalCoordinates, frameBuffer, depthBuffer, calculateColor);
}

void Rasterizator::DrawPolygonBarycentricTextureWithBillinearFiltration(const Triangle& polygon, std::vector<PointLightStruct> lightnings, 
	CoordinateStruct& CameraGlobalCoordinates, void* frameBuffer, void* depthBuffer, RGBQUAD color, TextureStruct& texture)
	...
	auto calculateColor = [&](float x, float y, float w0, float w1, float w2, const Triangle& polygon, CoordinateStruct& curPointInGlobal, CoordinateStruct& hitNormal, std::vector<PointLightStruct>& lightnings)
		{
			...
			RGBQUAD texelColor = _calculateBillinearColor (polygon, w0, w1, w2, texture);
			CoordinateStruct texelStruct = { texelColor.rgbBlue,texelColor.rgbGreen, texelColor.rgbRed };
			return _getLightedColor(texelStruct, hitColor);
		};
	...
	auto calculateColor = [&](float x, float y, float w0, float w1, float w2, const Triangle& polygon, CoordinateStruct& curPointInGlobal, CoordinateStruct& hitNormal, std::vector<PointLightStruct>& lightnings)
		{
			...
			CoordinateStruct normalFromMap = _getNormalMap(texelX, texelY, texture);

			int MapPixelIndex = (int)texelY * texture.textureWidth * texture.textureChannels + (int)texelX * texture.textureChannels;
			float ks = texture.specularTexturData[MapPixelIndex] / 255.0f;
			CoordinateStruct hitColor = calculatePhongLight(curPointInGlobal, normalFromMap, CameraGlobalCoordinates, lightnings, ks);
...
void Rasterizator::DrawPolygonPBRtexture(const Triangle& polygon, std::vector<PointLightStruct> lightnings, CoordinateStruct& CameraGlobalCoordinates, 
	void* frameBuffer, void* depthBuffer, RGBQUAD color, TextureStruct& texture, CoordinateStruct& material)
{
	auto calculateColor = [&](float x, float y, float w0, float w1, float w2, const Triangle& polygon, CoordinateStruct& curPointInGlobal, CoordinateStruct& hitNormal, std::vector<PointLightStruct>& lightnings)
		{
			std::tuple<float, float> texels = _getTexelIndexes(polygon, w0, w1, w2, texture);
			...
			RGBQUAD texelColor = _calculateBillinearColor(polygon, w0, w1, w2, texture);
			CoordinateStruct albedo = { texelColor.rgbBlue,texelColor.rgbGreen, texelColor.rgbRed };
			
			std::tuple<float, float, float> mrao = _getMRAO(polygon, w0, w1, w2, texture);

			CoordinateStruct hitColorPBR = calculatePBRLight(curPointInGlobal, normalFromMap, CameraGlobalCoordinates, lightnings, albedo,std::get<0>(mrao), std::get<1>(mrao), std::get<2>(mrao), material);
